...
namespace audio_driver
{
  ...
  {
  public:
    AudioDriverNAU8325Class() { i2c_default_address = 0x21; }

    bool setMute(bool mute) override
    {
      return nau.softMute(mute);
    }

    bool setVolume(int volume) override
    {
      int vol = mapVolume(volume, 0, 100, 0x00, 0xff);
      nau.setVolume(vol, vol);
      return true;
    }
  ...
  protected:
    PCBCUPID_NAU8325 nau = PCBCUPID_NAU8325(wire, i2c_default_address);
    int volume = 100;

    bool init_config(codec_config_t codec_cfg) override
    {
      int fs = getRate(codec_cfg.i2s.rate);
      int bits = getBits(codec_cfg.i2s.bits);
      return nau.begin(fs, bits);
    }
    ...
    {
      return mode == CODEC_MODE_DECODE || mode == CODEC_MODE_BOTH;
    }

    bool configureInterface(codec_mode_t mode, I2SDefinition iface) override
    {
      NAU8325_I2SFormat fmt = I2S_STD;
      switch (iface.fmt)
      {
      case I2S_NORMAL:
        /* code */
      ...
      return nau.setI2SFormat(fmt, NORMAL_BCLK);
    }

  private:
    int getRate(samplerate_t rate)
    {
      switch (rate)
      {
      case RATE_8K:
    ...
    int getBits(sample_bits_t bits)
    {
      switch (bits)
      {
      case BIT_LENGTH_16BITS
