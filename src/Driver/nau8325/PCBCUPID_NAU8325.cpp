...
uint16_t set_ratio = 0;  //declared as global
...
PCBCUPID_NAU8325::PCBCUPID_NAU8325(TwoWire &wire, uint8_t i2c_addr)
  : i2c(wire), i2c_addr(i2c_addr) {}

bool PCBCUPID_NAU8325::begin(uint32_t fs, uint8_t bits_per_sample, uint16_t ratio) {
  Serial.println("[NAU8325] beginDynamic() starting...");
  Serial.printf("  fs = %lu Hz\n", fs);
  Serial.printf("  ratio = %u Hz\n", ratio);
  Serial.printf("  bits = %u\n", bits_per_sample);

  if (bits_per_sample != 16 && bits_per_sample != 24 && bits_per_sample != 32) {
    Serial.printf("[NAU8325] Unsupported bit width: %u\n", bits_per_sample);
    return false;
  }
  ...
  Serial.println("[NAU8325] beginDynamic() complete - sound should play");
  return true;
}
...
bool PCBCUPID_NAU8325::applySampleRateClocks(const SRateAttr *srate, int n1_sel, int mclk_mult_sel, int n2_sel) {
  if (!srate || n1_sel < 0 || n1_sel >= 3 || n2_sel < 0 || n2_sel >= 5) {
    Serial.println("[NAU8325] Invalid N1/N2 divider index.");
    return false;
  }

  // --- Debug ---
  Serial.println("--- Debugging clock dividers ---");
  Serial.printf("n1_sel=%d (val=%d)\n", n1_sel, mclk_n1_div[n1_sel].val);
  Serial.printf("n2_sel=%d (val=%d)\n", n2_sel, mclk_n2_div[n2_sel].val);
  Serial.printf("mult_sel=%d\n", mclk_mult_sel);
...
bool PCBCUPID_NAU8325::chooseClockSource(int fs, int mclk,
  ...
  if (fs <= 0 || mclk <= 0) {
    Serial.println("[NAU8325] Invalid fs or mclk.");
    return false;
  }

  srate = getSRateAttr(fs);
  if (!srate) {
    Serial.printf("[NAU8325] Unsupported fs: %d\n", fs);
    return false;
  }
  ...
  if (ratio != NAU8325_MCLK_FS_RATIO_NUM) {
    n1_sel = 0;
    mult_sel = -1;  // Bypass
    Serial.printf("[NAU8325] Direct match: fs=%d, mclk=%d → N2=%d (Ratio=%d)\n",
  ...
  if (mclk_max > 0) {
    ...
    Serial.printf("[NAU8325] Matched via N1/N3 combo: fs=%d, mclk=%d → N1=%d, N3(mult)=%d, N2=%d\n",
                  fs, mclk, n1_sel, mult_sel, n2_sel);
    Serial.printf("[NAU8325] chooseClockSource(): fs=%d, mclk=%d\n", fs, mclk);

    return true;
  }

  Serial.printf("[NAU8325] Failed to match MCLK %d Hz with fs %d Hz\n", mclk, fs);
  return false;
}
...
bool PCBCUPID_NAU8325::configureClocks(int fs, int mclk) {
  ...
  if (!chooseClockSource(fs, mclk, srate, n1_sel, mult_sel, n2_sel)) {
    Serial.printf("[NAU8325] Failed to choose clock source for fs=%d, mclk=%d\n", fs, mclk);
    return false;
  }

  if (!applySampleRateClocks(srate, n1_sel, mult_sel, n2_sel)) {
    Serial.println("[NAU8325] Failed to apply sample rate clocks.");
    return false;
  }
  Serial.printf("[NAU8325] configureClocks(): fs=%u, mclk=%u\n", fs, mclk);
...
bool PCBCUPID_NAU8325::configureAudio(uint32_t fs, uint32_t mclk, uint8_t bits_per_sample) {
  ...
  if (!osr || osr->osr == 0 || (fs * osr->osr > CLK_DA_AD_MAX)) {
    Serial.println("[NAU8325] Invalid OSR or fs × OSR exceeds max.");
    return false;
  }
  ...
  switch (bits_per_sample) {
    ...
    case 32:
      ...
      break;
    default:
      Serial.printf("[NAU8325] Invalid bit width: %u\n", bits_per_sample);
      return false;
  }
  ...
  Serial.printf("[NAU8325] configureAudio(): fs=%u, mclk=%u, bits=%u\n", fs, mclk, bits_per_sample);

  return true;
}

bool PCBCUPID_NAU8325::setI2SFormat(NAU8325_I2SFormat format, NAU8325_ClockPolarity polarity) {
...
bool PCBCUPID_NAU8325::setSysClock(uint32_t freq) {
  if (freq < MASTER_CLK_MIN || freq > MASTER_CLK_MAX) {
    Serial.printf("[NAU8325] Invalid MCLK: %u Hz (allowed: %u - %u)\n", freq, MASTER_CLK_MIN, MASTER_CLK_MAX);
    return false;
  }

  this->mclk = freq;
  Serial.printf("[NAU8325] MCLK set to %u Hz\n", mclk);
  return true;
}
...
void PCBCUPID_NAU8325::initRegisters() {
  ...
  if (vref_val != 0xFFFF) {
    ...
    Serial.printf("[NAU8325] Invalid DAC Vref: %lu uV\n", dac_vref_microvolt);
  }
  ...
  if (vref_imp_val != 0xFFFF) {
    ...
    Serial.printf("[NAU8325] Invalid VMID impedance: %lu ohms\n", vref_impedance_ohms);
  }
...
bool PCBCUPID_NAU8325::writeRegister(uint16_t reg, uint16_t val) {
  Serial.printf("[WRITE] reg=0x%04X val=0x%04X\n", reg, val);
