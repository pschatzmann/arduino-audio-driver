...
static error_t ac101_write_reg(uint8_t reg_add, uint16_t data) {
  ...
  return i2c_bus_write_bytes(ac101_i2c_handle, i2c_addr_ac101, &reg_add,
                             sizeof(reg_add), (uint8_t*)send_buff,
                             sizeof(send_buff));
}

static error_t ac101_read_i2c(uint8_t devAddr, uint8_t reg_add, uint8_t* p_data,
                              size_t size) {
  return i2c_bus_read_bytes(ac101_i2c_handle, devAddr, &reg_add, sizeof(reg_add),
                            p_data, size);
}
...
inline void set_codec_clk(samplerate_t sampledata) {
  uint16_t sample_fre;
  switch (sampledata) {
    case RATE_8K:
      sample_fre = 8000;
...
inline uint16_t get_src_value(input_device_t input_device) {
...
inline int ac101_get_spk_volume(void) {
  int res;
...
inline error_t ac101_set_spk_volume(uint8_t volume) {
...
inline int ac101_get_earph_volume(void) {
  int res;
  res = ac101_read_reg(HPOUT_CTRL);
  return (res >> 4) & 0x3f;
}

inline error_t ac101_set_earph_volume(uint8_t volume) {
  if (volume > 0x3f) volume = 0x3f;
...
inline error_t ac101_set_output_mixer_gain(ac_output_mixer_gain_t gain,
...
inline error_t ac101_set_voice_mute(bool enable) {
  error_t res = 0;
...
inline error_t ac101_set_voice_volume(int volume) {
  error_t res;
...
inline error_t ac101_get_voice_volume(int* volume) {
  *volume = ac101_get_earph_volume();
  return 0;
}

inline error_t AC101_start(ac_module_t mode) {
  error_t res = 0;
...
inline error_t AC101_stop(ac_module_t mode) {
  error_t res = 0;
...
inline error_t ac101_init(codec_config_t* codec_cfg, i2c_bus_handle_t handle,
                          int addr) {
  error_t res = RESULT_OK;
  ac101_i2c_handle = handle;
  if (addr > 0) {
    i2c_addr_ac101 = addr;
  }
...
inline error_t ac101_deinit(void) {
  return ac101_write_reg(CHIP_AUDIO_RS, 0x123);  // soft reset
}

inline error_t ac101_ctrl_state_active(codec_mode_t mode, bool ctrl_state_active) {
  int res = 0;
  int es_mode_t = 0;
  ...
  if (!ctrl_state_active) {
    res = AC101_stop((ac_module_t)es_mode_t);
  } else {
    res = AC101_start((ac_module_t)es_mode_t);
  }
  return res;
}

inline error_t ac101_config_i2s(codec_mode_t mode, I2SDefinition* iface) {
  error_t res = 0;
  int bits = 0;
  int fmat = 0;
  int sample_fre = 0;
...
inline error_t AC101_i2s_config_clock(ac_i2s_clock_t* cfg) {
  error_t res = 0;
  uint16_t regval = 0;
  regval = ac101_read_reg(I2S1LCK_CTRL);
  regval &= 0xe03f;
