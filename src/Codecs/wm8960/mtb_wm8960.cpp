...
static bool _mtb_wm8960_config_default(uint8_t features) {
  ...
  for (uint32_t i = 0; i < sizeof(operations) / sizeof(_mtb_wm8960_operation_t);
       i++) {
...
bool mtb_wm8960_configure_clocking(uint32_t mclk_hz, bool enable_pll,
  ...
  /* Only two clock frequencies are supported for mclk if PLL is not being
   * used*/
  ...
  /* Set Clocking 1 */
  result =
      mtb_wm8960_write(WM8960_REG_CLK1, adc_div_mask | dac_div_mask |
