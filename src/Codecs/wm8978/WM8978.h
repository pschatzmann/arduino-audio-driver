...
// operations, all register values are saved locally. Write WM8978 register,
// synchronized to the local register values; register read returns locally
// stored value. Note: WM8978 register value is 9 bits, so use uint16_t.
...
class WM8978 {
 public:
  static constexpr int WM8978_ADDR = 0x1A;
  static constexpr uint8_t EQ1_80Hz = 0x00;
  static constexpr uint8_t EQ1_105Hz = 0x01;
  static constexpr uint8_t EQ1_135Hz = 0x02;
  static constexpr uint8_t EQ1_175Hz = 0x03;
  static constexpr uint8_t EQ2_230Hz = 0x00;
  static constexpr uint8_t EQ2_300Hz = 0x01;
  static constexpr uint8_t EQ2_385Hz = 0x02;
  static constexpr uint8_t EQ2_500Hz = 0x03;
  static constexpr uint8_t EQ3_650Hz = 0x00;
  static constexpr uint8_t EQ3_850Hz = 0x01;
  static constexpr uint8_t EQ3_1100Hz = 0x02;
  static constexpr uint8_t EQ3_14000Hz = 0x03;
  static constexpr uint8_t EQ4_1800Hz = 0x00;
  static constexpr uint8_t EQ4_2400Hz = 0x01;
  static constexpr uint8_t EQ4_3200Hz = 0x02;
  static constexpr uint8_t EQ4_4100Hz = 0x03;
  static constexpr uint8_t EQ5_5300Hz = 0x00;
  static constexpr uint8_t EQ5_6900Hz = 0x01;
  static constexpr uint8_t EQ5_9000Hz = 0x02;
  static constexpr uint8_t EQ5_11700Hz = 0x03;
  WM8978() {}
  ~WM8978() {}
...
inline uint8_t WM8978::Write_Reg(uint8_t reg, uint16_t val) {
...
// WM8978 init
// Return value: 0, initialization is normal; Other, error code
inline uint8_t WM8978::Init(void) {
  uint8_t res;
...
inline uint16_t WM8978::Read_Reg(uint8_t reg) { return REGVAL_TBL[reg]; }

// WM8978 DAC/ADC configuration
// adcen: adc enable(1)/disable(0)
// dacen: dac enable(1)/disable(0)
inline void WM8978::cfgADDA(uint8_t dacen, uint8_t adcen) {
  uint16_t regval;
...
inline void WM8978::cfgInput(uint8_t micen, uint8_t lineinen, uint8_t auxen) {
  uint16_t regval;
  regval = WM8978::Read_Reg(2);  // Read R2
  if (micen)
...
// (1)/off (0)
inline void WM8978::cfgOutput(uint8_t dacen, uint8_t bpsen) {
  uint16_t regval = 0;
...
inline void WM8978::setMICgain(uint8_t gain) {
  gain &= 0X3F;
...
// 3dB/Step
inline void WM8978::setLINEINgain(uint8_t gain) {
  uint16_t regval;
  gain &= 0X07;
...
// part) gain: 0~7, 0 means channel is disabled, 1~7, corresponds to -12dB~6dB,
// 3dB/Step
inline void WM8978::setAUXgain(uint8_t gain) {
  uint16_t regval;
  gain &= 0X07;
  regval = WM8978::Read_Reg(47);      // Read R47
...
// fmt: 0,LSB(right-aligned); 1,MSB(left-aligned); 2,Philips standard I2S;
//      3,PCM/DSP
// len: 0, 16 digits; 1, 20 digits; 2, 24 digits; 3, 32 digits
inline void WM8978::cfgI2S(uint8_t fmt, uint8_t len) {
  fmt &= 0X03;
...
inline void WM8978::setHPvol(uint8_t voll, uint8_t volr) {
  voll &= 0X3F;
...
// Set speaker volume
// volx: left channel volume (0~63)
inline void WM8978::setSPKvol(uint8_t volx) {
...
inline void WM8978::set3D(uint8_t depth) {
...
inline void WM8978::set3Ddir(uint8_t dir) {
  uint16_t regval;
  ...
  if (dir)
    ...
    regval &= ~(1 << 8);
  Write_Reg(18, regval);  // R18, the 9th bit of EQ1 controls the EQ/3D direction
}

// Set EQ1
// cfreq: cutoff frequency, 0~3, corresponding to: 80/105/135/175Hz
// gain: Gain, 0~24, corresponding to -12~+12dB
inline void WM8978::setEQ1(uint8_t cfreq, uint8_t gain) {
  uint16_t regval;
...
// gain: Gain, 0~24, corresponding to -12~+12dB
inline void WM8978::setEQ2(uint8_t cfreq, uint8_t gain) {
  uint16_t regval = 0;
  cfreq &= 0X3;  // Limited range
  if (gain > 24) gain = 24;
  gain = 24 - gain;
...
// gain: Gain, 0~24, corresponding to -12~+12dB
inline void WM8978::setEQ3(uint8_t cfreq, uint8_t gain) {
  uint16_t regval = 0;
  cfreq &= 0X3;  // Limited range
  if (gain > 24) gain = 24;
  gain = 24 - gain;
...
// gain: Gain, 0~24, corresponding to -12~+12dB
inline void WM8978::setEQ4(uint8_t cfreq, uint8_t gain) {
  uint16_t regval = 0;
  cfreq &= 0X3;  // Limited range
  if (gain > 24) gain = 24;
  gain = 24 - gain;
...
// gain: Gain, 0~24, corresponding to -12~+12dB
inline void WM8978::setEQ5(uint8_t cfreq, uint8_t gain) {
  uint16_t regval = 0;
  cfreq &= 0X3;  // Limited range
  if (gain > 24) gain = 24;
  gain = 24 - gain;
...
inline void WM8978::setALC(uint8_t enable, uint8_t maxgain, uint8_t mingain) {
  uint16_t regval;
...
inline void WM8978::setNoise(uint8_t enable, uint8_t gain) {
  uint16_t regval;
  ...
  regval = (enable << 3);
  regval |= gain;         // Set gain
  Write_Reg(35, regval);  // R35, noise gate setting
}

inline void WM8978::setHPF(uint8_t enable) {
  uint16_t regval;
...
inline bool WM8978::begin(i2c_bus_handle_t p_wire, int address) {
